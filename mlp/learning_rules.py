...
class AdamLearningRuleWithWeightDecay(GradientDescentLearningRule):
    ...
    def update_params(self, grads_wrt_params):
        ...
        # ηt = learning_rate / self.initial_learning_rate
        
        ηt = self.learning_rate / self.initial_learning_rate
        self.step_count += 1
        for param, mom_1, mom_2, grad in zip(
                self.params, self.moms_1, self.moms_2, grads_wrt_params):
            ...
            mom_1 *= self.beta_1
            mom_1 += (1 - self.beta_1) * grad
            # without it cannot pass the test.py, but it performs much better than that with it
            mom1 = mom_1 / (1 - self.beta_1**self.step_count)
            
            mom_2 *= self.beta_2
            mom_2 += (1 - self.beta_2) * grad**2
            # without it cannot pass the test.py, but it performs much better than that with it
            mom2 = mom_2 / (1 - self.beta_2**self.step_count)
            
            param *= (1 - ηt * self.weight_decay)
            param -= ηt * (self.learning_rate * mom1 /
                      ((mom2)**0.5 + self.epsilon))
