...
class CosineAnnealingWithWarmRestarts(object):
    ...
    def update_learning_rule(self, learning_rule, epoch_number):
        ...
        Args:
            ...
            epoch_number: Integer index of training epoch about to be run.
        """
#         epoch_number_period = epoch_number % (self.period_iteration_expansion_factor * self.total_epochs_per_period)
#         number_period = int(epoch_number / (self.period_iteration_expansion_factor * self.total_epochs_per_period))
#         learning_rule.learning_rate = self.min_learning_rate + 0.5 * ((self.max_learning_rate_discount_factor**number_period) * self.max_learning_rate - self.min_learning_rate) * (1 + np.cos(epoch_number_period/(self.period_iteration_expansion_factor * self.total_epochs_per_period) * np.pi))
        ...
        if self.period_iteration_expansion_factor == 1:
            ...
            number_period = np.ceil(np.log(1 + epoch_number*(self.period_iteration_expansion_factor-1)/ self.total_epochs_per_period) / np.log(self.period_iteration_expansion_factor))
            epoch_number_period = epoch_number - (self.period_iteration_expansion_factor**(number_period-1) - 1)*self.total_epochs_per_period / (self.period_iteration_expansion_factor-1) 
            discount = self.max_learning_rate_discount_factor **(number_period-1)
            decreased_max_lr = discount * self.max_learning_rate
            expand = self.period_iteration_expansion_factor ** (number_period-1)
            expanded_period = expand * self.total_epochs_per_period
            learning_rule.learning_rate = self.min_learning_rate + 0.5 * (decreased_max_lr - self.min_learning_rate) * (1 + np.cos(np.pi * epoch_number_period/ expanded_period))
